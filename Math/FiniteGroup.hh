...
/// Decomposition of finite (semi)group with enumerated elements
template<class G>
class GroupDecomposition {
public:
    /// Constructor
    GroupDecomposition(const G& g = {}) {
        build_cosets(g);
        build_cycles(g);
    }
    ...
    void display(std::ostream& o = std::cout) {
        o << "Group with " << this->getOrder() << " elements:\n";
        for(auto& kv: by_order) {
            o << "\t" << kv.second.size() << " of order " << kv.first << "\n";
            //for(auto& e: kv.second) o << "\t" << e << "\n";
        }
    }

    size_t getOrder() const { return left_cosets.size(); }

    map<size_t,set<size_t>> by_order;       ///< elements grouped by order
    vector<vector<size_t>> cycles;          ///< cyclic groups generated by each element
    vector<set<size_t>> left_cosets;        ///< left cosets for each element
    vector<set<size_t>> right_cosets;       ///< right cosets for each element

protected:
    /// build cosets
    void build_cosets(const G& g) {
        left_cosets.resize(g.getOrder());
        right_cosets.resize(g.getOrder());
        for(auto& e0: g) {
            auto i = g.idx(e0);
            for(auto& e1: g) {
                auto j = g.idx(e1);
                auto k = g.idx(g.apply(e0,e1));
    ...
    void build_cycles(const G& g) {
        ...
        for(i = 0; i < this->getOrder(); i++) by_order[cycles[i].size()].insert(i);
    }
};
