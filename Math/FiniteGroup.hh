...
/// All unique elements produced as combinations of input elements (sorted)
template<class G>
...
/// Decomposition of group from generators
template<class G>
class GeneratorGroupDecomposition {
public:
    typedef typename G::elem_t repr_t;
    typedef size_t elem_t;


    /// Constructor from generators
    GeneratorGroupDecomposition(const vector<repr_t>& gs): elems(span<G>(gs)), order(elems.size()) {
        build_CT();
        build_cycles();
    }
    ...
    void display(std::ostream& o = std::cout) {
        o << "Group with " << order << " elements:\n";
        for(auto& kv: by_order) o << "\t" << kv.second.size() << " of order " << kv.first << "\n";
    }

    /// element index
    elem_t idx(const repr_t& e) const { return std::lower_bound(elems.begin(), elems.end(), e) - elems.begin(); }
    /// pre-calculated group operator
    elem_t apply(elem_t a, elem_t b) const { return CT.find({a,b})->second; }

    const vector<repr_t> elems;             ///< enumeration of elements
    size_t order;                           ///< number of elements
    elem_t id;                              ///< identity element
    map<size_t,set<elem_t>> by_order;       ///< elements grouped by order

    map<pair<elem_t,elem_t>, elem_t> CT;    ///< Cayley Table a*b -> c
    vector<vector<elem_t>> cycles;          ///< cyclic groups generated by each element
    vector<set<elem_t>> left_cosets;        ///< left cosets for each element
    vector<set<elem_t>> right_cosets;       ///< right cosets for each element

protected:
    /// build Cayley Table
    void build_CT() {
        elem_t i = 0;
        left_cosets.resize(order);
        right_cosets.resize(order);
        for(auto& e1: elems) {
            elem_t j = 0;
            for(auto& e2: elems) {
                auto k = idx(G::apply(e1,e2));
                left_cosets[j].insert(k);
                right_cosets[i].insert(k);
                CT[{i,j++}] = k;
            }
            ++i;
        }
    ...
    void build_cycles() {
        ...
        for(elem_t i = 0; i < order; i++) by_order[cycles[i].size()].insert(i);
    }
