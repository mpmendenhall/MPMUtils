/// \file memvfs.c 2020-05-29 from: https://www.sqlite.org/src/file/ext/misc/memvfs.c
// modified M. P. Mendenhall (LLNL): shortened test code; cleanup documentation; do not register as default VFS
 ...
 **                  value when the connection closes (default false)
 **
 ** The ptr= and sz= query parameters are required.  If max= is omitted,
...
static int memWrite(
    ...
    MemFile* p = (MemFile*)pFile;
    if(iOfst + iAmt > p->sz) { // extending beyond previous file end?
        if(iOfst + iAmt > p->szMax) return SQLITE_FULL; // out of allocated space
        if(iOfst > p->sz) memset(p->aData + p->sz, 0, iOfst - p->sz); // zero-fill unwritten space
        p->sz = iOfst + iAmt; // record new size
    }
...
int sqlite3_memvfs_init(
    ...
    rc = sqlite3_vfs_register(&mem_vfs, 0);
